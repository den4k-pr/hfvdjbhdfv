document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkScroll() {
        ...
        if (window.innerWidth > 800) {
            ...
            // якщо залишилось ≤ 200px до низу → ховаємо
    ...
    // ==========================================
    // TIMER (Оновлено на 24 години з новим ключем)
    // ==========================================

    // Новий унікальний ключ для скидання старого кешу у користувачів
    const STORAGE_KEY = 'promo-card-timer-24h'; 
    const timerEls = footer.querySelectorAll('.promo-card-digit');
    const DAY_IN_MS = 24 * 60 * 60 * 1000; // 24 години в мілісекундах

    function getEndTime() {
        let end = localStorage.getItem(STORAGE_KEY);

        if (!end) {
            end = Date.now() + DAY_IN_MS; // 24 години відлік
    ...
    function updateTimer() {
        ...
        // 👉 якщо таймер закінчився — перезапускаємо знову на 24 години
        if (diff <= 0) {
            endTime = Date.now() + DAY_IN_MS;
